...
int call_snv_with_pl(struct mplp* saved_mutations, int* mut_ptr, struct mplp* my_mplp,
    ...
    int is_it_noisy = check_if_noisy(my_mplp, ref_idx, alt_idx, sample_mut_freq_limit, min_other_ref_freq_limit, cov_limit,
                                    current_ploidy, &noisiest_idx, &weakest_idx);
...
int get_max_non_ref_freq(struct mplp* my_mplp,double* val, int* idx, char* mut_base){
  ...
  */
  ...
  int check_if_noisy(struct mplp* my_mplp, int ref_idx, int alt_idx, double sample_mut_freq_limit,double min_other_ref_freq_limit, int cov_limit,
                    int* current_ploidy, int* noisiest_idx, int* weakest_idx){
...
int check_if_indel_noisy(struct mplp* my_mplp, double sample_mut_freq_limit, double min_other_ref_freq_limit, int cov_limit,
                                ...
                                char* most_frequent_indel, char* mut_type){
    //get index for ref base
    int base_2_idx[256];
    base_2_idx[ 'A' ] = A_IDX;base_2_idx[ 'C' ] = C_IDX;
    base_2_idx[ 'G' ] = G_IDX;base_2_idx[ 'T' ] = T_IDX;
...
int build_window_data(double ** window_data, char ** ch, int * pointer_wd, struct mplp* my_mplp){
    ...
    window_data[(*pointer_wd)][0] = (double) my_mplp->pos*1.0;
    window_data[(*pointer_wd)][1] = (double) my_mplp->counts[sample_idx][COV_IDX]*1.0;
    window_data[(*pointer_wd)][2] = noise_freq;
    window_data[(*pointer_wd)][3] = 0.0; //if there is any data, reinitialize with 0, otherwise it stays -42
    window_data[(*pointer_wd)][4] = 0.0;
...
int shift_window(double ** window_data, char ** ch, int * pointer_wd, int ws, int rows, int shift,
    ...
    for(i=0;i<ws;i++)
    {
      if (window_data[i][0] > 0) //valid position
      {
        window_data[i][3] = window_data[i][3] + avg_cov;
        window_data[i][4] = window_data[i][4] + 1;
      }
    }
    ...
    for (i=0;i<ws;i++)
    {
      if (i<shift && window_data[i][2] > min_noise && window_data[i][2] < 1-min_noise) //these are filtered, then printed, diploid and triploid cov data updated
      {
          ...
          {
            (*dip_cov_total) = (*dip_cov_total)+window_data[i][3]/window_data[i][4];
            (*dip_count) = (*dip_count)+1;
          }
          // if (((window_data[i][2] > TRIP_MIN_FREQ_LOW && window_data[i][2] < TRIP_MAX_FREQ_LOW) || (window_data[i][2] > TRIP_MIN_FREQ_HIGH && window_data[i][2] < TRIP_MAX_FREQ_HIGH))
          // && window_data[i][3]/window_data[i][4] > MIN_COV_FOR_EST && window_data[i][3]/window_data[i][4] < MAX_COV_FOR_EST)
          if ((window_data[i][2] > TRIP_MIN_FREQ_HIGH && window_data[i][2] < TRIP_MAX_FREQ_HIGH) && window_data[i][3]/window_data[i][4] > MIN_COV_FOR_EST && window_data[i][3]/window_data[i][4] < MAX_COV_FOR_EST)
          {
            (*trip_cov_total) = (*trip_cov_total)+window_data[i][3]/window_data[i][4];
