...
int main(int argc, char** argv)
{
    ...
    //sample names
    int n_sample_names=argc-10;
    char** sample_names= (char**) malloc(n_sample_names * sizeof(char*));
    int i=0;
    int j;
    for(i=0;i<n_sample_names;i++) sample_names[i]=argv[10+i];
    ...
    //FILE* test_f = fopen("/Users/ribli/unique_mutation/data2/dbg.input","r"); //this is for testing only
    ...
    while ((line_size = getline(&line, &len, stdin)) != -1) {
        ...
        call_snv_with_pl(potential_mutations,&mut_ptr,&my_mplp,
